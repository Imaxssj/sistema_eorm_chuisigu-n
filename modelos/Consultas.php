...
class Consultas
{
	...
	{
		$sql = "SELECT a.id as idalumno, a.name,a.lastname,a.image  FROM alumn a INNER JOIN alumn_team alt ON a.id=alt.alumn_id WHERE alt.team_id='$idgrupo'";
		return ejecutarConsulta($sql);
	}

	public function cantidadg($user_id, $idgrupo)
	{
		$sql = "SELECT COUNT(*) total_alumnos  FROM alumn a INNER JOIN alumn_team alt ON a.id=alt.alumn_id WHERE alt.team_id='$idgrupo'";
		return ejecutarConsulta($sql);
	}
	...
	public function cantidadgrupos($idusuario)
	{
		if ($idusuario === '1' || $idusuario === '2')
			$sql = "SELECT idgrupo,nombre, idusuario, favorito FROM team ";
		else
